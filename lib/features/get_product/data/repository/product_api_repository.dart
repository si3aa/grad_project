    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (response.data is Map && response.data['data'] != null) {
          ...
          return data.map((json) => AllproductDataModel.fromJson(json)).toList();
        } 
        ...
        // If we can't find a list in the response, log the error and return an empty list
          ...
          developer.log('API Response format unexpected: ${response.data}', name: 'ProductAPI');
          ...
          if (response.data is Map) {
            ...
            for (var key in (response.data as Map).keys) {
              if (response.data[key] is List) {
                ...
                developer.log('Found list in response under key: $key', name: 'ProductAPI');
                return data.map((json) => AllproductDataModel.fromJson(json)).toList();
              }
            }
          }
          developer.log('Could not find product list in response', name: 'ProductAPI');
          return [];
        }
      } else {
        developer.log('API Error: ${response.statusCode} - ${response.statusMessage}', name: 'ProductAPI', error: response.data);
      ...
      if (e is DioException) {
        ...
        if (e.response != null) {
          developer.log('Dio error response: ${e.response?.data}', name: 'ProductAPI');
        }
      }
